...
export default {
	...
	methods: {
		...
		//教辅版本列表
		version() {
			this.$api.version({ subject_id: this.subject_id, page: this.page }).then(res => {
				console.log(res);
		...
		tochaxun() {
			...
			if (arr.length > 0) {
				this.$api.add_my_textbook({ token: this.token, textbook_ids: arr.toString(), subject_id: this.subject_id }).then(res => {
					console.log(res);
					if (res.code == 200) {
						uni.navigateBack();
					} else {
