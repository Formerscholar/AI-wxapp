		...
		<view class="student" v-show="type==4">
			<view class='fix'>
				...
				<view class="" v-if="select">
					<view class="content">
						...
						<view v-if='is_more==0&&exercises_list.length!=0' class='is_more'>
							没有更多错题了
						</view>
				...
				<view class="list" v-else>
					...
					<view v-if='is_more2==0&&errorbook_list.length!=0' class='is_more'>
						没有更多错题本了
					</view>
			...
			<view class="teach" v-show="type==3">
				...
				<view class="content" v-if="select">
					<view class="item2" v-for="(item,i) of exercises_list" :key='i' @click="selected_topic(i)">
						<view v-if="update" :class="{'b-n':item.select}" class='b-default'>
						</view>
						<view class=""> 
							<rich-text :nodes="changeStyle(item.content)"></rich-text>
						</view>
					...
					<view class="b-btn">
						<view class='leftCon'>
							<view class="selectAll" @click="selectAll()">
								<view v-if="update" :class="{'b-n':all==false}" class='b-default' style='margin: 0;'>
								</view>
								<text>全选</text>
							</view>
							<button @click="delete_error_exercises()">删除</button>
			...
			<uni-popup ref="popupJoin" type="center">
				<view class="maskJoin">
					...
					<view v-else class="listJoin none">
						您还没有生成试卷!
					</view>
					<view class='btnCon'>
	...
	export default {
		...
		methods:{
			...
			fasong(){
				...
				if(uni.getStorageSync('type')==3){
					this.$api.get_teacher_text(data)
					.then(res=>{
						console.log(res)
						if(res.code==200){
							this.email=''
