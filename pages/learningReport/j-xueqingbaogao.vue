<template>
  <view>
    ...
    <view class="qiun-charts" v-if="analysisList.length && analysisList.length != 0">
      <view class="item_box" v-for="(item,index) in analysisList" :key="item.id" @click="handleAnalysClick(item)">
        <view class="top_box">
    ...
    <view class="hotTitle" v-if="exercises_list.length != 0">
      ...
      <view class="hotTitleCon">
        <image src="https://aictb.oss-cn-shanghai.aliyuncs.com/wx_xcx/bg/hot.png"></image>
        <text>热门易错题型</text>
      </view>
    ...
    <view class="list">
      ...
      <view v-if="is_more == 0 && exercises_list.length != 0" class="is_more">没有更多{{ type == 3 ? '试题' : '错题' }}了</view>
    </view>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        pieData: [],
        colorList: ['#EC6941', '#F19EC2', '#00A0E9', '#7E6B5A', '#80C269', '#0068B7', '#f1c40f', '#c0392b', '#8e44ad',
          '#EA2027'
        ],
