	...
	<view>
		...
		<view class='cardCon' v-if='classList'>
			<view v-for='(item,i) of classList' :key='i' class="card">
				<view class='flex'>
					<view class='up' @click="add(item)"><image src='//aictb.oss-cn-shanghai.aliyuncs.com/wx_xcx/icon/grade.png'/><view class='upCon'>{{item.team_name}}</view></view>
					<view class='up'><image src="//aictb.oss-cn-shanghai.aliyuncs.com/wx_xcx/icon/invent.png"/><button class='upCon' @click="join(item.classid)">加入班级</button></view>
				</view>
				<view class='flex'>
...
<style lang="scss">
	...
	.text{
		...
		text:nth-of-type(2),text:nth-of-type(3),text:nth-of-type(4){
			margin-left:30rpx;
		}
		text{
			font-size: 24rpx;
