    ...
    <view class="card" v-for="(item, i) of list" :key="i">
      <view class="flex">
        <view @click="add(item)" class="up">
          <view class="className">
            ...
            <image class='guan' v-if="user_id == item.teacher_id" src="https://aictb.oss-cn-shanghai.aliyuncs.com/wx_xcx/bg/admin.png"
              mode="widthFix"></image>
          </view>
      ...
      <view class="flex">
        <view class="down">
          ...
          <!-- #ifdef MP-WEIXIN -->
          <button class="downCon" open-type="share" :data-name="item.get_grade.name+item.name" :data-class_id="item.classid" data-it="stu">邀请学生</button>
          <!-- #endif -->
          <!-- #ifdef APP-PLUS -->
        ...
        <view class="down">
          ...
          <!-- #ifdef MP-WEIXIN -->
          <button class="downCon" open-type="share" :data-team_id="item.id" :data-name="item.get_grade.name+item.name"
            :data-class_id="item.classid" data-it="ter">邀请老师</button>
          <!-- #endif -->
          <!-- #ifdef APP-PLUS -->
  ...
  export default {
    ...
    onShareAppMessage(e) {
      ...
      if (e.target.dataset.it == 'stu') {
        path =
          `/pages/login/share?id=${user.id}&school=${this.school_name}&name=${e.target.dataset.name}&teacher_name=${user.true_name}&class_id=${e.target.dataset.class_id}`;
      } else {
        path =
          `/pages/login/share_ter?id=${user.id}&school=${this.school_name}&name=${e.target.dataset.name}&teacher_name=${user.true_name}&class_id=${e.target.dataset.class_id}&team_id=${e.target.dataset.team_id}`;
      }
    ...
    methods: {
      ...
      InviteStudent(item, index) {
        ...
        if (index == 3) {
          this.sharePath =
            `pages/login/share_ter?id=${user.id}&name=${item.get_grade.name+item.name}&class_id=${item.classid}&school=${this.school_name}&teacher_name=${user.true_name}&team_id=${item.id}`
        } else {
          this.sharePath =
            `pages/login/share?id=${user.id}&name=${item.get_grade.name+item.name}&class_id=${item.classid}&school=${this.school_name}&teacher_name=${user.true_name}`
        }
